...
import { tickle } from '../../utils/gif.json'

export default new client.command({
    structure: new SlashCommandBuilder()
        .setName('щекотать')
        .setDescription('Пощекотать пользователя')
    ...
    run(client, interaction) {
        ...
        const gif = tickle[Math.floor(Math.random() * tickle.length)];
        const user = interaction.user;
        const target = interaction.options.getUser('юзер');
        const embed = new EmbedBuilder()
            .setAuthor({ name: 'Команда: щекотать' })
            .setDescription(`${user}, пощекотал(а) ${target}\n\n${textContent}`)
            .setImage(gif)
            .setTimestamp()
