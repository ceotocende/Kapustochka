...
import { press } from '../../utils/gif.json'

export default new client.command({
    structure: new SlashCommandBuilder()
        .setName('прижать')
        .setDescription('прижать')
    ...
    run(client, interaction) {
        ...
        const gif = press[Math.floor(Math.random() * press.length)];
        const user = interaction.user;
        const target = interaction.options.getUser('юзер');
        const embed = new EmbedBuilder()
            .setAuthor({ name: 'Команда: прижать' })
            .setDescription(`${user}, прижал(а) ${target}\n\n${textContent}`)
            .setImage(gif)
            .setTimestamp()
