...
import { blush } from '../../utils/gif.json'

export default new client.command({
    structure: new SlashCommandBuilder()
        .setName('покраснеть')
        .setDescription('покраснеть')
    ...
    run(client, interaction) {
        ...
        const gif = blush[Math.floor(Math.random() * blush.length)];
        const user = interaction.user;
        const target = interaction.options.getUser('юзер');
        const embed = new EmbedBuilder()
            .setAuthor({ name: 'Команда: покраснеть' })
            .setDescription(`${target} смущает ${user}\n\n${textContent}`)
            .setImage(gif)
            .setTimestamp()
