...
import { wink } from '../../utils/gif.json'

export default new client.command({
    structure: new SlashCommandBuilder()
        .setName('подмигнуть')
        .setDescription('подмигнуть')
    ...
    run(client, interaction) {
        ...
        const gif = wink[Math.floor(Math.random() * wink.length)];
        const user = interaction.user;
        const target = interaction.options.getUser('юзер');
        const embed = new EmbedBuilder()
            .setAuthor({ name: 'Команда: подмигнуть' })
            .setDescription(`${user}, подмигнул(а) ${target}\n\n${textContent}`)
            .setImage(gif)
            .setTimestamp()
