...
import { touch } from '../../utils/gif.json'

export default new client.command({
    structure: new SlashCommandBuilder()
    .setName('потрогать')
        .setDescription('потрогать')
    ...
    run(client, interaction) {
        ...
        const gif = touch[Math.floor(Math.random() * touch.length)];
        const user = interaction.user;
        const target = interaction.options.getUser('юзер');
        const embed = new EmbedBuilder()
            .setAuthor({ name: 'Команда: потрогать' })
            .setDescription(`${user}, трогает ${target}\n\n${textContent}`)
            .setImage(gif)
            .setTimestamp()
